...
static void	give_color_rainbow(mlx_image_t *g_img, uint32_t x, uint32_t y, int iterations)
{
	int 			i;
	int				j;
	size_t 			k;
	uint32_t 		lst[] = {0x9400D3FF, 0x9400D3FF, 0x4B0082FF, 0x0000FFFF, 0x00FF00FF, 0xFFFF00FF, 0xFF7F00FF, 0xFF0000FF};

	i = MAX_ITERATIONS;
	k = 1;
	if (iterations == i)
		mlx_put_pixel(g_img, x, y, lst[0]);
	j = i - 15;
	while (i > 0)
	{
